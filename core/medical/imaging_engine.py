...
from core.medical.fhir.bundle import build_bundle
from core.medical.fhir.generators import FHIRGeneratorContext, _meta
...
class ImagingEngine:
    ...
    def generate(
        ...
        for i in range(num_studies):
            ...
            if trial_patients:
                ...
                patient_encounters = [
                    e for e in trial_encounters if e.get("subject", {}).get("reference") == f"Patient/{patient['id']}"
                ]
    ...
    def _build_fhir_imaging_study(
        self, full_study: dict, patient_info: dict | None, encounter_ref: dict | None
    ) -> dict:
        ...
        study_date, study_time = study["study_date"], study["study_time"]
        started = (
            f"{study_date[:4]}-{study_date[4:6]}-{study_date[6:]}"
            f"T{study_time[:2]}:{study_time[2:4]}:{study_time[4:]}Z"
        )
        resource = {
        ...
        if patient_info:
            resource["subject"] = {
                "reference": f"Patient/{patient_info['id']}",
                "display": patient_info["name"].replace("^", " "),
            }
        else:
